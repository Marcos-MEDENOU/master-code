...
class Article extends Database
{
    ...
    private $id, $title, $image, $code_html, $category_id, $user_id, $created_at;
    ...
    public function getOneArticleById($id)
    {
        ...
        // Requête SQL
        $sql = "SELECT `articles`.article_id, `articles`.article_image, `articles`.article_title, `articles`.code_html, `categories`.category_name, `users`.user_pseudo, `articles`.created_at, `articles`.updated_at FROM `blog`.articles INNER JOIN `blog`.categories ON `categories`.category_id = `articles`.category_id INNER JOIN `blog`.users ON `users`.user_id = `articles`.user_id WHERE `articles`.article_id =?;";
    ...
    public function addArticle($image, $title, $code_html, $category_id, $user_id, $created_at)
    {
        ...
        // Requête préparée
        $statement = $conn->prepare($sql);
        $result = $statement->execute([$this->image, $this->title, $this->code_html, $this->category_id, $this->user_id, $this->created_at]);
        return $result;
    }
