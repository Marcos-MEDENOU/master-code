...
class User extends Database
{
    ...
    public function updateOneUser($id, $role, $time)
    {
        $conn = $this->connect();
        $this->role = $role;
        $this->id = $id;
         ...
         * $sql, pour les requêtes vers la base de données
         */
        $sql = "UPDATE `blog`.users set `users`.user_role = :role, `users`.updated_at = :temps WHERE `users`.user_id = :id";
        /**
         * $stmt, pour recupérer la requête préparée
         */
        $stmt = $conn->prepare($sql);
    ...
    // Pour connaître le nombre d'utilisateur inscrit sur le blog
    public function countUser()
    {
         ...
         * $sql, pour les requêtes vers la base de données
         */
        $sql = "SELECT COUNT(`users`.user_pseudo) AS Nombre FROM users;";
        /**
         * $stmt, pour recupérer la requête préparée
         */
        $stmt = $conn->prepare($sql);
