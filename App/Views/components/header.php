...
<header class="py-8 flex justify-between items-center">
	<nav
		...
		<div class="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl m-0 p-0">
			<a href="?goto=<?= $this->datacrypt('home') ?>&action=<?= $this->datacrypt('viewHome') ?>"
				class="flex items-center"> <img src="ressources/images/web.svg" class="mr-3 h-6 sm:h-9 App-logo"
					alt="Flowbite Logo" />
				<span class="self-center text-2xl font-semibold dark:text-white name-website p-0 m-0">
					<span class="text-cliford font-sans font-extrabold tracking-wide"  style="letter-spacing: 0.15rem;">CODEUR</span> 
					<span class="text-rose-600 p-0 m-0" style="letter-spacing: 0.05rem;">INFO</span> 
				</span> 
			</a>
			...
			<div class="flex items-center lg:order-2">
				<div>
					<?php if (isset($_SESSION['Auth'])) { ?>
						<div id="profile"
							class="mr-2 l-7 h-10 w-10 hover:ring-4 user cursor-pointer relative ring-blue-700/30 rounded-full bg-cover bg-center bg-[url('ressources/images/user.png')]">
						</div>

						<div id="profile_menu" class="show">
							<div class="mt-3 drop-down  w-48 overflow-hidden bg-white rounded-md shadow absolute">
								<ul>

									<?php if (isset($_SESSION['Auth']['role']) && $_SESSION['Auth']['role'] == '0') { ?>
										<li
											class="px-3 py-3 text-sm font-medium flex items-center space-x-2 hover:bg-gray-200 ">
											<a href="?goto=<?= $this->datacrypt('home') ?>&action=<?= $this->datacrypt('sms') ?>"
												class="flex gap-2">
									...
									<li
										class="px-3 py-3 text-sm font-medium flex items-center space-x-2 hover:bg-gray-200 ">
										<a href="?goto=<?= $this->datacrypt('home') ?>&action=<?= $this->datacrypt('comment') ?>"
											class="flex gap-2">
											<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
												stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
												<path stroke-linecap="round" stroke-linejoin="round"
				...
				<a href="?goto=<?= $this->datacrypt('home') ?>&action=<?= $this->datacrypt('articles') ?>"
					class="ml-3 hidden md:block text-2xl text-white bg-rose-600 hover:bg-rose-700 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 dark:bg-blue-600 dark:hover:bg-orange-700 focus:outline-none dark:focus:ring-blue-800">Découvrir
					nos articles</a>
			...
			<div class="ml-10 flex justify-between items-center hidden justify-between items-center w-full lg:flex lg:w-auto lg:order-1"
				id="mobile-menu-2">
				<ul class="flex flex-col mt-4 font-medium lg:flex-row lg:space-x-8 lg:mt-0">
					...
					<li><a href="?goto=<?= $this->datacrypt('home') ?>&action=<?= $this->datacrypt('articles') ?>"
							class="sm:block md:hidden block py-2 pr-4 pl-3 text-gray-700 border-b border-gray-100 hover:text-orange-400  lg:border-0 lg:hover:text-orange-400 lg:p-0 dark:text-gray-400 lg:dark:hover:text-white dark:hover:bg-gray-700 dark:hover:text-white lg:dark:hover:bg-transparent dark:border-gray-700">Découvrir
							nos articles</a>
					</li>
				</ul>
